...
class IrModel(models.Model):
    ...
    abstract = fields.Boolean(string="Abstract Model", store=True)
...
class KsCustomReport(models.Model):
    ...
    ks_is_enable_multi_company = fields.Boolean("Multi Company", default=False)
    ks_ir_rule_id = fields.Many2one("ir.rule", string="Rule")
    ks_company_field_id = fields.Many2one("ir.model.fields", string="Company Field")
    ...
    def ks_onchange_on_ks_cr_query_type(self):
        if self.ks_cr_query_type == 'custom_query':
            self.ks_is_enable_multi_company = False
            if self.ks_ir_rule_id:
                self.ks_ir_rule_id.unlink()
            if self.ks_company_field_id:
                self.ks_company_field_id.unlink()
    ...
    def write(self, vals):
        ...
        for res in self:
            ...
            if any([vals.get('ks_cr_column_ids'), vals.get('ks_cr_query_type'), vals.get('ks_custom_query'), vals.get('ks_is_enable_multi_company')]):
    ...
    def unlink(self):
        for res in self:
            ...
            rec.ks_ir_rule_id.unlink()
            rec.ks_company_field_id.unlink()
    ...
    def _ks_cr_menu_action(self, rec):
        ...
        domain = []
        if rec.ks_is_enable_multi_company:
            domain = [('x_company_id', 'in', self._context.get('allowed_company_ids'))]
    ...
    def _ks_create_db_table(self, rec):
        ...
        if rec.ks_cr_query_type == 'field_chain':
            ...
            if rec.ks_is_enable_multi_company and rec.ks_model_id.field_id.filtered(lambda x: x.name == 'company_id'):
                self._ks_create_ir_rule(rec)
            if not rec.ks_is_enable_multi_company:
                self.ks_unlink_ir_rule(rec)
        else:
    ...
    def _ks_create_ir_rule(self, rec):

        if not rec.ks_company_field_id:
            values = {
                'name': 'x_company_id',
                'ttype':'many2one',
                'field_description': "Company Id",
                'model_id': rec.ks_cr_model_id.id,
                'state': 'manual',
                'relation': 'res.company',
            }
            rec.ks_company_field_id = self.env['ir.model.fields'].sudo().create(values)

        # if not rec.ks_ir_rule_id:
        #     val = {
        #         'name': rec.ks_cr_model_id.name + " Multi-Company",
        #         'model_id': rec.ks_cr_model_id.id,
        #         'domain_force': "[('x_company_id', 'in', company_ids)]",
        #     }
        #     rec.ks_ir_rule_id = self.env['ir.rule'].sudo().create(val)

    @api.onchange('ks_is_enable_multi_company')
    def onchange_ks_is_enable_multi_company(self):
        for rec in self:
            if not rec.ks_is_enable_multi_company:
                rec.ks_cr_action_id.domain = []
                if rec.ks_company_field_id:
                    rec.ks_company_field_id.unlink()
                # rec.ks_ir_rule_id.unlink()
                # rec._ks_create_db_table(rec)

    def ks_unlink_ir_rule(self, rec):
        # if rec.ks_ir_rule_id:
        #     rec.ks_ir_rule_id.unlink()
        if rec.ks_company_field_id:
            rec.ks_company_field_id.unlink()
    ...
    @api.model
    def ks_get_company_wise_data(self,companyId):
        try:
            all_records = self.env['ks_custom_report.ks_report'].sudo().search([])
            for rec in all_records:
                if rec.ks_is_enable_multi_company:
                    domain = [('x_company_id', 'in', companyId)]
                    rec.ks_cr_action_id.domain = domain
        except:
            pass
